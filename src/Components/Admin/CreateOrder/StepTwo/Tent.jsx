...
import { TentLoadingType, TentContainer, Cargo, CargoCategory } from "./data";
import PropTypes from 'prop-types';
import { IoIosArrowDown } from "react-icons/io";

const Tent = ({ setFormData, formData, setAuth, setIsNext, auth }) => {
  ...
  const handleOptionClick = (key, option) => {
    ...
    if (key === "containerType") {
      newFormData.ncontainerType = Object.keys(TentContainer).find(
        (k) => TentContainer[k] === option
      );
    } else if (key === "cargoCategory") {
      ...
      newFormData.nloadingType = Object.keys(TentLoadingType).find(
        (k) => TentLoadingType[k] === option
      );
    }
  ...
  const dataSources = {
    containerType: TentContainer,
    loadingType: TentLoadingType,
      ...
      <form onSubmit={handleSubmit} className="max-w-700">
        <div className="d-flex flex-row w-50 flex-nowrap row">
          <div className="mb-3">
            ...
            <div className="dropdown input-container" ref={dropdownRefs.containerType}>
              <input
                type="text"
        ...
        <div className="d-flex flex-row w-50 flex-nowrap row">
          <div className="mb-3">
            <label htmlFor="loadingType"  className={`mb-1 font-15 ${
              !fieldValidity.loadingType ? "text-danger" : ""
            }`}>
              Loading type *
            </label>
            <div className="dropdown input-container" ref={dropdownRefs.loadingType}>
              <input
                type="text"
                className={`input-border form-control h-40 bg-form pointer pe-4 ${
                  !fieldValidity.loadingType ? "border-danger" : ""
                }`}
                id="loadingType"
                name="loadingType"
                value={formData.loadingType}
                readOnly
                onClick={() => handleDropdownToggle("loadingType")}
              />
              <IoIosArrowDown className="input-arrow" onClick={() => handleDropdownToggle("loadingType")}/>

              <ul
                className={`dropdown-menu w-100 p-0 -mt-5 input-border ${
                  dropdownState.loadingType ? "show" : ""
                }`}
              >
                <li>
                  <input
                    ...
                    className="form-control input search-input"
                    placeholder="&#128269; Search..."
                    value={searchTerms.loadingType}
                    onChange={(e) =>
                      handleSearchInputChange("loadingType", e.target.value)
                    }
                  />
                </li>
                {filteredOptions("loadingType").map((option) => (
                  <li key={option}>
                    <button
                      type="button"
                      className="btn w-100 text-start font-15"
                      onClick={() => handleOptionClick("loadingType", option)}
                    >
                      {option}
                    </button>
                  </li>
          ...
          <div className="mb-3">
            ...
            <div className="dropdown input-container" ref={dropdownRefs.cargoCategory}>
              <input
                type="text"
                className={`input-border form-control h-40 bg-form pointer pe-4 ${
        ...
        <div className="d-flex flex-row w-50 flex-nowrap row">
          <div className="mb-3">
            ...
            <div className="dropdown input-container" ref={dropdownRefs.cargoType}>
              <input
                type="text"
                className={`input-border form-control h-40 bg-form pointer pe-4 ${
        ...
        <div className="position-relative mb-3 w-50 row">
          <label
            htmlFor="price"
            className={`mb-1 font-15 ${
          ...
          <div className="d-flex align-items-center">
            ...
            <span className="separator font-23">|</span>
            <input
              type="number"
